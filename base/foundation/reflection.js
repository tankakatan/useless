...
_.tests.reflection = {
	...
	'readSource': function () {
		_.readSource ($scriptsRoot + 'base/foundation.js', function (text) {
			$assert (text.length > 0) })

		_.readSourceLine ($scriptsRoot + 'base/foundation.js', 0, function (line) {
	...
	'Prototype.$sourceFile': function () {
		if (Platform.NodeJS) {
			var Proto = $prototype ()
			$assert (Proto.$sourceFile, 'base/foundation/reflection.js') } }
}
			...
			return _.map (array, function (entry) {
				return _.extend (entry, {
							...
							thirdParty:		(entry.file.indexOf ($scriptsRoot) == -1) ||
											...
											(entry.file.indexOf ('node_modules') != -1) }) }) },
